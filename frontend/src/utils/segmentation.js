...
function buildTrackPoints(detail) {
  if (!detail) return []
...
function buildSegments(points, mode) {
  if (!points.length) return { segments: [], legend: [] }

  if (mode === 'kilometers') {
    const bounds = [0]
    let next = 1000
    for (let i = 1; i < points.length; i++) {
      if (points[i].dist >= next) {
        bounds.push(i)
        next += 1000
      }
    }
    bounds.push(points.length - 1)
    return buildBoundarySegments(points, bounds, (i) => `Km ${i + 1}`)
  }
  ...
  if (mode === 'laps') {
    ...
    for (let i = 1; i < points.length - 1; i++) {
      const sinceLap = points[i].dist - points[bounds[bounds.length - 1]].dist
      const distFromStart = haversineMeters(startLat, startLon, points[i].lat, points[i].lon)
      if (sinceLap >= lapMinDist && distFromStart < 25) {
        bounds.push(i)
      }
    }
    if (bounds[bounds.length - 1] !== points.length - 1) bounds.push(points.length - 1)
    return buildBoundarySegments(points, bounds, (i) => `Lap ${i + 1}`)
  }

  if (mode === 'speed') {
    const labels = ['Very slow', 'Easy', 'Moderate', 'Fastest']
    return buildBucketSegments(points, (p) => (p.speed == null ? NaN : p.speed * 3.6), labels)
  }

  if (mode === 'hr') {
    const labels = ['Low HR', 'Aerobic', 'Tempo', 'Max effort']
    return buildBucketSegments(points, (p) => (p.hr == null ? NaN : p.hr), labels)
  }
...
export { palette, haversineMeters, buildTrackPoints, buildSegments, toChartBands }
