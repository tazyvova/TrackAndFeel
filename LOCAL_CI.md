...
- **Docker Socket**: `act` requires access to the Docker socket. The Dev Container is configured to mount it via the `docker-outside-of-docker` setup (mounted directly in `docker-compose.extend.yml`).
