...
- `PREVIEW_DOMAIN` – the wildcard domain (e.g. `preview.example.com`) pointing DNS `*.PREVIEW_DOMAIN` to the product server. Defaults to `preview.local` for local runs to avoid empty Compose interpolations.
